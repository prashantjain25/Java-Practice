...
//This Algorithm just traverses the nodes and does not find shortest path
class Graph {

    private Node[] vertices; // stores the nodes of the graph
    private int size; // number of nodes in the graph
    private DijkstraWithoutAdjency makeArray;

    public Graph(int size) {
        this.size = size;
        vertices = new Node[size];     addNodes();
        makeArray = new DijkstraWithoutAdjency(size);
...
//The CRUX
    ...
    private void applyDijkstraAlgorithm(Node sourceNode) {
        ...
        while (!makeArray.isEmpty()) {
            Node visitedNode = makeArray.remove();
            ...
            while (connectedEdge != null) {
                ...
                if (neighbour.state == State.NEW) {
                    makeArray.add(neighbour);
                    neighbour.state = State.IN_Q;
                }
                // updating [relaxing] the costs of each non visited neighbor
                // node if its
                // have been made lesser.
                if (neighbour.state != State.VISITED && ((connectedEdge.weight
                        + visitedNode.cost) < neighbour.cost)) {
        ...
        for (int i = 0; i < size; i++) {
            if (vertices[i].cost != Integer.MAX_VALUE) {
                System.out.println("distance from " + sourceNode.name + " to "
                        + vertices[i].name + " is " + vertices[i].cost);
            } else {
                System.out.println(vertices[i].name + " is not reachable from "
                        + sourceNode.name);
            }
        }
    }
 ...
 *
 */
public class DijkstraWithoutAdjency {
    Node[] queue;
    int maxSize;
    int rear = -1, front = -1;

    DijkstraWithoutAdjency(int maxSize) {
